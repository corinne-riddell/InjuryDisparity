...
The gun household prevalences were estimated previously and can be found in Table 1 of the following article (see the column for 2004):
Monuteaux MC, Lee LK, Hemenway D, Mannix R, Fleegler EW. Firearm ownership and violent crime in the us: An ecologic study. *American journal of Preventive Medicine*. 2015;49(2):207-214.

```{r gun_ownership_data}
...
- Hit send.
- After a few seconds, a dataset will be created of the number and rates of firearm-related homicide in each state-race dyad. We saved these data in the Data directory under the title "Homicide_Firearms_NH_Males_Age-adjusted_2008-2016.txt"
...
```{r append_US_and_state_wide_datasets}
...
```{r save-data, echo=F}
save(CDC_Males_AllAges_0816_wide, file = "./Data/Males_AllAges_0816.Rdata")
```